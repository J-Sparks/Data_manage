...
```{r}
library(dplyr)
wide_enrcrsdata <- read.csv("G:/Shared drives/HMCSE-PAM Lab/DATA 202001/V1FreqOrderwith9crs_MASTER_CRSCSE_allcourse_tier.csv")
...
degspring2020 <- read.csv("G:/Shared drives/HMCSE-PAM Lab/DATA 202001/STUDENT_DEGREE_SPRING20.csv") %>%
  ...
  group_by(STU_ID) %>% filter(Deg_Desc=="Bachelor", .by_group=TRUE) %>% mutate(number_Bachelor=row_number(), endDay=rep("01")) %>% 
   select(1:2,Deg_College, Deg_TermDate,Deg_Desc ,number_Bachelor)
...
enrspring2020 <- read.csv("G:/Shared drives/HMCSE-PAM Lab/DATA 202001/STU_ENROLLMENT_SPRING20.csv") %>% 
  filter(Stu_StudentTypeCode == "B" & # for FTIC
           ...
           Stu_DEMO_TIME_FRAME >= 200805 ) %>% # data quality since 2015
  group_by(STU_ID) %>% 
  filter(str_detect(Stu_Term,"Fall...")) %>% 
  separate(Stu_AdmissionTerm, into = c("Term","Cohort"),sep=" ")
  
wide_info_deg <- merge(enrspring2020, degspring2020,by="STU_ID", all.x = TRUE)
wide_info_deg$Stu_TermDate <- as.Date.character(wide_info_deg$Stu_TermDate)
wide_info_deg$Deg_TermDate <- as.Date.character(wide_info_deg$Deg_TermDate)

head(wide_info_deg,10)
# create col for data
wide_info_deg$Days <- as.character(round(wide_info_deg$Deg_TermDate - as.Date(wide_info_deg$Stu_TermDate)))

wide_info_deg$Months <- as.character(round(wide_info_deg$Deg_TermDate - as.Date(wide_info_deg$Stu_TermDate))/(365.25/12))

wide_info_deg$Years <- as.character(round(wide_info_deg$Deg_TermDate - as.Date(wide_info_deg$Stu_TermDate))/(365))
wide_info_deg$Ontime_gradu <- ifelse(wide_info_deg$Years < 4.00, "Ontime","Overtime")

wide_info_deg$Semester <- as.character(round(wide_info_deg$Deg_TermDate - as.Date(wide_info_deg$Stu_TermDate))/(365/3))

wide_info_deg$Start_FTIC_Term  <- wide_info_deg$Stu_TermDate
head(wide_info_deg,10)
```
```{r}
xtabs(~wide_info_deg$Deg_College+wide_info_deg$Ontime_gradu+wide_info_deg$Cohort+wide_info_deg$Deg_Desc)
```

```{r}
MASTER_CRSCSE_allcourse_tier <- read.csv("G:/Shared drives/HMCSE-PAM Lab/DATA 202001/V1FreqOrderwith9crs_MASTER_CRSCSE_allcourse_tier.csv") %>% filter(COHORT_YEAR<=20202019)
library(ggplot2)
...
mycollege <- MASTER_CRSCSE_allcourse_tier %>% 
  ...
  summarise(totalFTIC = n()) %>% 
  arrange(-totalFTIC, .by_group=TRUE) ;mycollege
...
do.call(rbind,lapply(MASTER_CRSCSE_allcourse_tier[6:7],tblFun))
