...
```{r}
getwd()
setwd("C:/Users/jsparks3/Documents/Repeated_F")
P3.Future.500.The.Dataset <- read.csv("~/Repeated_F/P3-Future-500-The-Dataset.csv",
...
```{r}
 library(readr)
 V1FreqOrderwith9crs_MASTER_CRSCSE_allcourse_tier <- read_csv("~/Repeated_F/V1FreqOrderwith9crs_MASTER_CRSCSE_allcourse_tier.csv")

mydata1 <- V1FreqOrderwith9crs_MASTER_CRSCSE_allcourse_tier
mydata <- mydata1 %>% 
  select(1:5,11:14, 20:23,29:70,78:84) 
 
mydata <- data.table(mydata)
mydata$aveGPAInd <- as.factor(mydata$aveGPAInd)
...
mydata[ , by=aveGPAInd, .N]
class(mydata);mydata$aveGPA <- mydata$aveGPAInd
```
#library
