...
tt
 ...
 library(readr)
 STUDENT_DEGREE_SPRING20 <- 
   read_csv("C:/Users/jk99/Downloads/STUDENT_DEGREE_SPRING20.csv") 
 
 library(dplyr)
degUWF_master <- STUDENT_DEGREE_SPRING20 %>% 
  filter(Deg_Desc == "Bachelor") %>% # only bachelor
  select(1:4,Deg_Gender,Deg_Ethnicity,Deg_College,Deg_Department,Deg_Desc,
         Deg_MajorMinorInd,Deg_MajorIndicator,Deg_TermGranted) %>% 
  filter(Deg_DEMO_TIME_FRAME >= 201508) 

write.csv(degUWF_master,"degUWF_master.csv")


degUWF_master1 <- degUWF_master %>%
  group_by(STU_ID) %>% 
  arrange(Deg_TermGranted, .by_group=TRUE) %>% 
  mutate(ID_index = row_number())
...
############################
# Degree by enthnicity by TIME#
############################


# get ID from enr dataset 
# and filter for demo time and create index

deg_ID <- select(degUWF_master1, "STU_ID",Deg_TermGranted )
enr_ID_for_merge <- STU_ENROLLMENT_SPRING20 %>% 
  select(1:4,Stu_Classification,Stu_MajorDesc,Stu_Ethnicity,
                           Stu_StudentType,Stu_AdmissionRecentTypeDesc,Stu_CampusDesc,
                           Stu_College,Stu_Department,Stu_OnlineCourseDelivery,Stu_GenderCode) %>% 
  group_by(STU_ID) %>% 
  arrange(Stu_DEMO_TIME_FRAME, .by_group=TRUE) %>% 
  mutate(ID_index=row_number()) %>% 
  filter(ID_index== 1)# keep the first time in UWF
unique(enr_ID_for_merge$ID_index)

enr_deg_UWF_MASTER <- merge(degUWF_master1,enr_ID_for_merge, by="STU_ID", all.x = TRUE)
write.csv(enr_deg_UWF_MASTER,"enr_deg_UWF_MASTER.csv")
